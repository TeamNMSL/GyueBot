...
async def singleCard(m:model.GroupMsgModel):
    #哪有这种道理啊（恼
    if m.getUserid()=="2114573521"and gs.get_value("isDeloresCued")==False:
        msg = m.CQ.At(m.getUserid())  + f"还我白毛狐狸😭还给我还给我还给我😭😭😭耳朵可拆卸的猫不准抽卡😭"
        await sendMsg(m, msg)
        gs.set_value("isDeloresCued",True)

    random.seed(int(datetime.datetime.now().timestamp()*int(m.getUserid())*cardkey))
    ...
    msg = m.CQ.At(m.getUserid()) + "\n" + m.CQ.Img(
        f"{gs.path_botImgTarotDir}/{fileName}") + "\n" + f"{card_key}:{card_value}\n只帮你抽卡，自己找人解去\n或者你可以让chatgpt帮你解"
    await sendMsg(m,msg)
...
def cardSort():
    ...
    reslist=[]
    for i in cards:
        cardlist.append(i+".jpg")
    pth = fileopt.dirRedirect(gs.path_botImgTarotDir)
